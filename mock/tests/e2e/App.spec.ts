...
test('after I input view url, output page changes', async ({ page }) => {
  // CHANGED
  await page.goto('http://localhost:8000/');
  await page.getByLabel('Login').click();
...
test('after I input "load_file", page changes', async ({ page }) => {
  // CHANGED
  await page.goto('http://localhost:8000/');
  await page.getByLabel('Login').click();
  ...
  expect(firstChild).toEqual("successfully loaded");
});
...
test('after I input search url, output page changes', async ({ page }) => {
  ...
  await page.getByLabel('Command input').click();
  await page.getByLabel('Command input').fill('search 2 English');
  await page.getByRole('button', {name: 'Submitted 1 times'}).click()
  ...
  expect(firstChild).toEqual("Tom21English");
});
